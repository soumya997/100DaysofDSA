...
bool condition(vector<int> v, int n, int m) {
    int cnt = 1;
    int last_barn = v[0];
...
int barn_search(vector<int> v, int n) {
    /*
        1. find search space
        2. apply bs on it
        3. find the condition by comparing that bs result
        with the given data.
    */

    int s = 0;
    int e = v[v.size() - 1] - v[0];
    int ans = 0;
    while (s <= e) {
        int mid = (s + e) / 2;
        if (condition(v, n, mid)) {
            ans = mid;
            s = mid + 1;
        }
        else {
