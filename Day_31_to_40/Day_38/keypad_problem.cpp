...
char keypad[][10] = {"","","abc","def","ijk","lmn","oprs","tuv","wxyz"};
void generate_key(char s[],char out[],int i,int j){
    if(s[i] == '\0'){
        out[j] = '\0';
        cout<<out<<endl;
        return;
    ...
    if(digit == 1 or digit ==0){
        generate_key(s,out,i+1,j);
    }


    for(int k=0;keypad[digit][k]!='\0';k++){
        out[j] = keypad[digit][k];
        generate_key(s,out,i+1,j+1);
    }
    return;
